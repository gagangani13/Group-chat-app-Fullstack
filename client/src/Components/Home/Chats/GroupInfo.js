...
const GroupInfo = () => {
  ...
  async function getParticipants() {
    const response = await axios.get(
      `http://3.232.208.74:5000/getParticipants?groupId=${groupId}`,
  ...
  async function addToGroup(e) {
    ...
    const response = await axios.post(
      `http://3.232.208.74:5000/joinGroup?groupId=${groupId}`,
      { participantId: id },
      { headers: { Authorization: idToken } }
    );
    const data = await response.data;
  ...
  async function makeAdmin() {
    const response = await axios.patch(
      `http://3.232.208.74:5000/makeAdmin?groupId=${groupId}`,
  ...
  async function removeParticipant() {
    const response = await axios.delete(
      `http://3.232.208.74:5000/removeParticipant/${options.id}?groupId=${groupId}`,
      { headers: { Authorization: idToken } }
    );
    const data = await response.data;
    try {
  ...
  async function exitGroup() {
    ...
    if (userResponse && userResponse.toLowerCase() === "yes") {
      const response = await axios.delete(
        `http://3.232.208.74:5000/exitGroup/${groupId}`,
        { headers: { Authorization: idToken } }
      );
      const data = await response.data;
      try {
  ...
  async function editGroupName() {
    if (!newGroupNameRef.current.value) {
      ...
      const response = await axios.patch(
        `http://3.232.208.74:5000/editGroupName?groupId=${groupId}`,
